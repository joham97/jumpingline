...
public class MyRectangle {
	...
	public MyVector collisionIntersection(MyLine line) {
		MyVector shortest = null;
		for (MyLine edge : this.edges) {
			MyVector colVector = line.collides(edge);
			if (colVector != null && (shortest == null || new MyVector(line.x1, line.y1).dst(colVector) < new MyVector(line.x1, line.y1).dst(shortest))) {
				shortest = colVector;
			}
		}
		return shortest;
